...
population.addEventListener('click', function () {
    ...
    populationResult.innerHTML = "";
    languageResult.innerHTML = "";
    // languageResult.style.display = 'none';

    const newTable = document.createElement("table");
    newTable.innerHTML = "<thead><th>Name</th><th>color</th><th>Population</th></thead>";
    for (const { name, population } of populated) {

        const newRow = document.createElement("tr");
        const countries = document.createElement("td");
        const color = document.createElement("td");
        color.style.setProperty('--percentage', `${(population / populated[0].population) * 100}%`);
        color.className = "container"
...
mostLanguages.addEventListener('click', function () {
    ...
    newTables.innerHTML = "<thead><th>Language</th><th>color</th><th>Count</th></thead>";
    for (const { language, count } of resultAr) {
        const newRow = document.createElement("tr");
        const languageData = document.createElement("td");
        const color = document.createElement("td");
        color.style.setProperty('--percentage', `${(count / resultAr[0].count) * 100}%`);
        color.className = "container";
